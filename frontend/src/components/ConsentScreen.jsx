...
export default function ConsentScreen({ onConsent, authStatus, onAuthUpdate }) {
  ...
  const showOAuthPanel = user || authStatus?.googleOAuthEnabled || import.meta.env.DEV;

  const handleLogout = async () => {
    setSigningOut(true);
    try {
      await logout();
      onAuthUpdate({ authenticated: false, googleOAuthEnabled: authStatus?.googleOAuthEnabled ?? false, user: null });
    } catch {
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-[760px] flex-col justify-center px-6 py-10">
        <div className="space-y-8">
          ...
          <div className="grid gap-3 text-[14px] leading-[1.55] text-secondary">
            ...
            <div className="rounded-[10px] border border-border-subtle bg-surface p-4">
              If you choose optional Google sign-in, the prototype stores your Google account ID, email, display name,
              avatar URL and login time for authentication only. The study responses remain linked to your participant code.
            </div>
          </div>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[13px] font-medium text-primary">
                      {user ? `Signed in as ${user.display_name || user.email}` : 'Optional Google sign-in'}
                    </p>
                    <p className="mt-1 text-[12px] leading-[1.45] text-muted">
                      {user
                        ? 'Your study responses still use the participant code, not your name.'
                        : 'Not required for the study. Used only if the prototype is configured to demonstrate OAuth.'}
                    </p>
                  </div>
                </div>
                ...
                {user ? (
                  ...
                  <button
                    ...
                    className="rounded-full border border-border-subtle px-4 py-2 text-[12px] font-medium text-secondary transition-colors hover:border-border-strong hover:text-primary disabled:cursor-not-allowed disabled:text-muted"
                  >
                    {authStatus?.googleOAuthEnabled ? 'Continue with Google' : 'Google sign-in not configured'}
                  </button>
                )}
              </div>
            </div>
